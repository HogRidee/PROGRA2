...
void incrementarEspacio(char **&codigos, char **&nombres, char ***&cursos, int n, int &cap){
    cap += INCREMENTO;
    if(codigos == nullptr){
        codigos = new char*[cap+1]{};
        nombres = new char*[cap+1]{};
        cursos = new char**[cap+1]{};
    }
    else{
        char **auxCodigo = new char*[cap+1]{};
        char **auxNombre = new char*[cap+1]{};
        char ***auxCursos = new char**[cap+1]{};
        for(int i = 0; i < n; i++){
            auxCodigo[i] = codigos[i];
...
//            incrementarEspacio(cursos, n, cap);
            incrementarEspacio(codigos, nombres, cursos, n, cap);
        }
        codigos[n] = tcodigo;
        nombres[n] = tnombre;
